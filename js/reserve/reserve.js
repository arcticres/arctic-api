...
(function(factory) {
    ...
    var defaults = {
        ...
        templates: {
            field: function (obj) {
                ...
                return '<div class="control-group">' + (obj.id ? '<label class="control-label" for="' + obj.id + '">' : '<div class="control-label">') + (obj.html ? obj.label : _escape(obj.label)) + (obj.id ? '</label>' : '</div>') + '<div class="controls">' + obj.controls + (obj.help ? '<span class="help-inline">' + _escape(obj.help) + '</span>' : '') + '</div></div>';
            },
            ...
            bookAddOnReservation: function (obj) {
                ...
                return '<select name="' + obj.id + '" id="' + obj.id + '"><option value="false"' + (obj.has ? "" : " selected") + '>No</option><option value="true"' + (obj.has ? " selected" : "") + '>Yes (add ' + _formatCurrency(obj.amount) + ')</option></select>';
            },
            bookAddOnGuest: function (obj) {
                return '<input type="number" name="' + obj.id + '" id="' + obj.id + '" size="3" maxlength="3" class="input-small" value="' + _escape(obj.has) + '" min="0" step="' + _escape(obj.step || 1) + '"' + (obj.def ? ' data-isdefault="true"' : '') + '> (' + _formatCurrency(obj.amount) + ' per guest)';
            },
    ...
    function widget(instance_options) {
        ...
        function _buildBookFields(pricing, guests) {
            // make sets
            var sets = {":pl": '<div class="pricing-levels">'}, other = "";
